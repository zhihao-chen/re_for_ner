...
from ReNER.tools.file_util import cached_path
...
class BertPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        ...
        # Need to unity it when we separate the dimensions of hidden and emb
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str) or (sys.version_info[0] == 2 and isinstance(config.hidden_act, unicode)):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
        ...
        if masked_lm_labels is not None:
            ...
            if not output_att:
                ...
                return masked_lm_loss, att_output  # noqa
        else:
            if not output_att:
                return prediction_scores
            else:
                return prediction_scores, att_output  # noqa
...
class BertCropModel(BertPreTrainedModel):

    def __init__(self, config: BertConfig):
        super(BertCropModel, self).__init__(config)
